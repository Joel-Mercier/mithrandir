...
export async function runGraph(): Promise<void> {
  ...
  const lines = [
    ...
    `  ${DIM}6.${RESET} Bazarr       ${DIM}(subtitles — connects to Radarr + Sonarr, optional)${RESET}`,
    ...
    `  ${DIM}9.${RESET} Seerr        ${DIM}(requests — connects to Radarr, Sonarr, Jellyfin, optional)${RESET}`,
    `  ${DIM}10.${RESET} Profilarr   ${DIM}(quality profiles — connects to Radarr, Sonarr, optional)${RESET}`,
    ``,
    ...
    `  ${i("wireguard")}       ${i("excalidraw")}   ${i("omnitools")}   ${i("openwebui")}`,
    `  ${i("actualbudget")}    ${i("sure")}   ${i("affine")}   ${i("n8n")}   ${i("penpot")}`,
    ``,
  ];
