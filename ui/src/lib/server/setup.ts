...
import { generateCompose, generateGatusExtraHosts } from "@mithrandir/cli/lib/compose";
import { getSwapInfo, ensureSwap, formatSwapSize } from "@mithrandir/cli/lib/swap";
import { isRcloneInstalled, installRclone } from "@mithrandir/cli/lib/rclone";
...
export const resolveAppDependencies = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ resolved: string[]; autoAdded: string[] }> => {
    await ensureSession();
...
export const setupHttps = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await shell(
      "docker",
      ["build", "-t", "caddy-duckdns", "-f", `${caddyDir}/Dockerfile`, caddyDir],
    ...
    await shell(
      ...
      [
        "-c",
        `cat > "${caddyDir}/404.html" << 'HTML_EOF'\n${page404}HTML_EOF`,
      ],
      { sudo: true },
    );
...
export const autoSetupApp = createServerFn({ method: "POST" })
  ...
  .handler(
    async ({
      ...
      const app = APP_REGISTRY.find((a) => a.name === appName);
      if (!app) return { success: false, warnings, error: `App not found: ${appName}` };
      ...
      try {
        ...
        if (appName === "qbittorrent") {
          ...
          if (!creds) throw new Error("No temporary password found in container logs");
          const client = createQBittorrentClient({ baseUrl });
          const login = await client.auth.login(creds.username, creds.password);
          if (!login.success) throw new Error("Login failed (IP may be banned)");
        ...
        if (appName === "prowlarr") {
          ...
          if (!apiKey) throw new Error("Could not read API key from config.xml");
            ...
            try {
              await client.downloadClient.create({
                ...
                fields: [
                  ...
                  { order: 2, name: "username", label: "Username", value: username },
                  { order: 3, name: "password", label: "Password", value: password },
                ],
              });
            } catch (err: unknown) {
              warnings.push(err instanceof Error ? err.message : String(err));
        ...
        if (appName === "jellyfin") {
          ...
          if (!info.StartupWizardCompleted) {
            ...
            await client.startup.updateUser({ Name: username, Password: password });
        ...
        if (appName === "seerr") {
          ...
          if (!apiKey) throw new Error("Could not read API key from settings.json");
          ...
          if (hasApp("jellyfin")) {
            ...
            try {
              const libs = await client.jellyfinSettings.getLibraries({ sync: true });
              ...
              const enableIds = [movieLib?.id, tvLib?.id]
                ...
                .join(",");
              if (enableIds) {
                await client.jellyfinSettings.getLibraries({ enable: enableIds });
              }
            } catch (err: unknown) {
              warnings.push(err instanceof Error ? err.message : String(err));
            }
