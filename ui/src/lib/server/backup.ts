		...
		if (existsSync(archiveDir)) {
			...
			if (result.exitCode === 0 && result.stdout.trim()) {
				...
				for (const date of dates) {
					...
					const lsResult = await shell("ls", ["-1", dateDir], {
						ignoreError: true,
					});
					...
					const size =
						duResult.exitCode === 0
							? (duResult.stdout.trim().split(/\s+/)[0] ?? "—")
							: "—";
					const encrypted = files.some((f: string) =>
						f.endsWith(ENCRYPTED_EXT),
					);
...
export const verifyBackup = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }): Promise<{ success: boolean; output: string }> => {
		...
		const result = await shell("/usr/local/bin/mithrandir", args, {
			cwd: projectRoot,
			ignoreError: true,
			timeout: 120000,
		});
