...
/**
 * Read the Radarr API key from BASE_DIR/radarr/config/config.xml.
 * Returns null if the file does not exist or the key cannot be parsed.
 */
export async function getRadarrApiKey(baseDir: string): Promise<string | null> {
  const configPath = join(baseDir, "radarr", "config", "config.xml");
...
export interface RadarrClientOptions {
  /** Base URL of the Radarr instance (default: http://localhost:7878) */
  baseUrl?: string;
  /** API key — if omitted, getRadarrApiKey() must have been called first */
  apiKey: string;
}
...
async function request<T>(
  ...
  if (!res.ok) {
    ...
    throw new Error(`Radarr API ${method} ${path} → ${res.status} ${res.statusText}: ${text}`);
  }
...
export function createRadarrClient(options: RadarrClientOptions) {
  const baseUrl = (options.baseUrl ?? "http://localhost:7878").replace(/\/$/, "");
