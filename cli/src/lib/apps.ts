...
export const APP_REGISTRY: AppDefinition[] = [
  ...
  },
  {
    ...
    description: "Self-hosted plant management system",
    image: "ghcr.io/danielbrendel/hortusfox-web:latest",
    containerName: "hortusfox_app",
    capacity: { performance: "low", storage: "low", note: "Plant tracker with MariaDB database" },
    additionalContainers: ["hortusfox_db"],
    port: 8089,
    configSubdir: "db",
    needsDataDir: false,
    rawCompose: (envConfig: EnvConfig) => {
      const baseDir = envConfig.BASE_DIR;
      const dbPassword = envConfig.HORTUSFOX_DB_PASSWORD ?? "hortusfox";
      const adminEmail = envConfig.HORTUSFOX_ADMIN_EMAIL ?? "admin@example.com";
      const adminPassword = envConfig.HORTUSFOX_ADMIN_PASSWORD ?? "password";
      const lines = [
        `services:`,
        `  hortusfox-app:`,
        `    image: ghcr.io/danielbrendel/hortusfox-web:latest`,
        ...
        `      - 8089:80`,
        `    volumes:`,
        `      - hortusfox-images:/var/www/html/public/img`,
        `      - hortusfox-logs:/var/www/html/app/logs`,
        `      - hortusfox-backup:/var/www/html/public/backup`,
        `      - hortusfox-themes:/var/www/html/public/themes`,
        `      - hortusfox-migrate:/var/www/html/app/migrations`,
        `    depends_on:`,
        `      - hortusfox-db`,
        `    restart: unless-stopped`,
        ``,
        ...
        `    volumes:`,
        `      - ${baseDir}/hortusfox/db:/var/lib/mysql`,
        `    healthcheck:`,
    ...
    secrets: [
      {
        envVar: "HORTUSFOX_ADMIN_EMAIL",
        prompt: "HortusFox admin email",
        required: true,
      },
      {
        envVar: "HORTUSFOX_ADMIN_PASSWORD",
        prompt: "HortusFox admin password",
        sensitive: true,
        required: true,
      },
      {
        envVar: "HORTUSFOX_DB_PASSWORD",
        prompt: "HortusFox database password",
        sensitive: true,
      },
    ],
  },
...
export const APP_CATEGORIES: AppCategory[] = [
  ...
  },
  {
    label: "Household",
    value: "household",
    description: "CookCLI, HortusFox",
    apps: ["cookcli", "hortusfox"],
  },
  {
    label: "Utilities",
    value: "utilities",
