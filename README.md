...
- [ ] We're going to upgrade the CLI setup command in cli/src/commands/setup.tsx to handle the initial setup of the installed apps automatically thanks to the API wrappers we added in cli/src/lib. After the app installation step with docker compose we want to prompt the user and ask him if he wants to use the automatic setup or if he wants to manually setup the apps. If he chooses the automatic setup, we will first make sure that all the API's credentials are retrieved and that their clients are initialized correctly. Then we will start by setting up qBittorrent, then Prowlarr, then Radarr, then Sonarr, then Lidarr, then Jellyfin and Seerr last. Make sure to only attempt to setup the app that have been installed by the user.

I want to add another step to the setup command of this CLI in @cli/src/commands/setup.tsx. I already started it it's in the AutoSetupAppsStep function. I want to ask the user if he wants to use the automatic setup or if he wants to manually setup the apps. If he chooses the automatic setup, we will first make sure that all the API's credentials are retrieved and that their clients are initialized correctly. Then we will start by setting up qBittorrent, then Prowlarr, then Radarr, then Sonarr, then Lidarr, then Jellyfin and Seerr last. Make sure to only attempt to setup the app that have been installed by the user. All the API methods needed are in the API clients that I imported. I know what I started isn't correct don't hesitate to correct already written code in this function.

1. qBittorrent
 - prompt the user for a username and password (save them and use them as defaults for the next username/password prompts)
 -  set the downloads save path to /data/downloads, set automatic torrent management to true and set the new WebUI username and password with the setPreferences method
2. Prowlarr
 - prompt the user for a username and password (use defaults)
 - use the hostConfig update method to set the authentication method to Forms and set the username and password
 - use the application create method several times to setup apps with lidarr, sonarr and radarr, this requires the urls (with local ip) and the api keys
3. Radarr
 - prompt the user for a username and password (use defaults)
 - use the hostConfig update method to set the authentication method to Forms and set the username and password
 - use the downloadClient create method to set the download client to qBittorrent
 - use the rootFolder create method to set the root folder to /data/media/movies
4. Sonarr
 - prompt the user for a username and password (use defaults)
 - use the hostConfig update method to set the authentication method to Forms and set the username and password
 - use the downloadClient create method to set the download client to qBittorrent
...
6. Jellyfin
 - prompt the user for a username and password (use defaults)
 - prompt for the remaining required information
 - use the startup api endpoints to setup Jellyfin with the values provided
 - create a movie type library called "Movies" and a tv show type library called "TV Shows", the paths are set to /data/media/movies and /data/media/tv
7. Seerr
 - setup the jellyfin settings
 - get the jellyfin users
 - use the import-from-jellyfin endpoint to import the user from Jellyfin
 - setup the radarr settings
 - setup the sonarr settings
